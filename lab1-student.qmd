...
*This dataset investigates the effect of __vitamin C___ on tooth growth in __guinea pigs__.*

*The two supplement delivery methods include OJ (_orange juice___) and VC (__ascorbic acid__).*

*`ToothGrowth` is a data frame with _60__observations and __3__ variables.*
...
##Two Sample T Test on ToothGrowth
```{r}
t.test(ToothGrowth$len, y = ToothGrowth$dose, alternative = c("two.sided", "less", "greater"), mu = 0, paired = FALSE, var.equal = FALSE, conf.level = 0.95)
```
1. With a p value of 2.2e-16 there is strong evidence that the true difference in means is not 0. So we reject the null hypothesis.
2. We are 95% confident that the interval (15.66453 19.62881) contains the true difference in means.
